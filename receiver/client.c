...
int main(int argc, char * argv[])
{
  ...
  sin_t.sin_addr.s_addr = inet_addr("127.0.0.1");
 ...
 if((send(s_tcp, "Start\n", strlen("Start\n")+1, 0)) <0)
  ...
  gtk_window_set_title (GTK_WINDOW (window), "Multicast Streaming");
  gtk_window_set_default_size (GTK_WINDOW (window), 300, 300);
  gdk_color_parse("light yellow", &color);		//Set color of GUI window
  g_signal_connect (window, "destroy", G_CALLBACK (gtk_main_quit), NULL);
  
  
  lab = gtk_label_new ("Click on one of the above buttons to start streaming");
  grid = gtk_grid_new ();
